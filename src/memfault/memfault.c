...
LOG_MODULE_REGISTER(applayer_memfault, CONFIG_APPLAYER_MEMFAULT_LOG_LEVEL);
...
static void button_handler(uint32_t button_states, uint32_t has_changed)
{
    ...
    {
        int err =
            memfault_metrics_heartbeat_add(MEMFAULT_METRICS_KEY(Button1ToggleCount), 1);
        if (err)
        {
            LOG_ERR("Failed to increment Button1ToggleCount");
        }
        else
        {
            LOG_INF("Button1ToggleCount incremented");
        }
    }
...
void start_memfault(void)
{
    ...
    while (1)
    {
        int taken = k_sem_take(&nw_connected_sem, K_NO_WAIT);
        if (taken)
        {
            LOG_INF("nw_connected_sem not available");
            k_yield();
